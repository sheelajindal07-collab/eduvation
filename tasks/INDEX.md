...
Lockboard (lead only): `RESET` line goes here before the shared local stack is reset. Currently: **APPLY PENDING, URGENT SECURITY FIX (Phase 1a lead, 2026-09-22)** - migration `0015_ai_identity_binding.sql` merged to `main` (closes a real anon-caller identity-hash oracle in `ai_reserve`/`ai_budget_remaining`, AI-4 follow-up). Additive `create or replace function` only, no table/RLS change, no reset - low collision risk, but posting per protocol. Applying shortly; will post again once done.

...
- [x] AUTH-6 - My Plan and Save controls, guest mode, new files only, cross-user lines [W3a, DONE, merged; sets session_state="account" for a real signed-in student -- the first page to do so, finally making AUTH-3's already-built "Sign out" nav branch reachable] -- one fix round closed two HIGH findings: a live CSRF bypass letting a cookie-less cross-origin visitor mint an unsolicited guest session via /my-plan/save, and a trust-label omission that silently hid an overdue-for-recheck warning Compare/Requirements would show for the same fact. Disclosed, not carded, tracked as a fast-follow: the "My Plan" nav destination still reads "soon" (_nav.html is a shared, frozen contract file, outside this card's own files) even though the route now works.
...
- [x] AUTH-3 - Sign-in, sign-up (flagged off), sign-out pages [float, DONE, merged; one fix round closed a live gap: MINOR_ACCOUNTS_ENABLED (CLAUDE.md's named human-review gate for minor sign-up) was dead code in sign_up() -- now fails closed] -- disclosed, not carded, tracked for AUTH-6/AUTH-14: no page yet sets session_state="account", so a signed-in student has no discoverable sign-out link until one of those builds a real protected page; bounded today by SIGNUP_ENABLED=false and the ~1h cookie lifetime
...
- [x] AUTH-9 - Student data export [P2, DONE, merged; GET /account, GET /account/export -- own-RLS-scoped client only, no-store, payload never logged; live cross-user isolation and guest/reviewer-only-session edge cases independently re-verified] -- disclosed, not carded: (1) tests/db/access_matrix.py's EXPORT placeholder still can't see this route (a pre-existing nested-router introspection gap under the pinned FastAPI version, unrelated to this task); (2) new finding -- anon still holds full table-level grants on `consents`/`plan_actions` (and, wider, `guardian_consents`/`safeguarding_staff`/`safeguarding_flags`/`student_accounts`), the same gap SEC-6's 0016 closed for `student_profiles`/`saved_plans`/`reviewers` but never reached these; RLS alone currently blocks it (live-confirmed three independent ways), no live leak, but no grant-layer backstop either -- tracked as a follow-up migration-owner task, not fixed here
...
- [x] PUB-2 - Migration 0017 (real number, not the stale planning-slot 0011): identity enforcement (created_by/reviewed_by forced+frozen to auth.uid(), NULL-safe self-approval), content-hash trigger, edit-during-review reverts to draft, approved_draft_version, immutable source_versions, frozen source identity once referenced by a published claim, claim_tier (critical/cycle/annual) with a critical-tier reviewer-authorisation gate, first private Storage bucket (source-evidence, reviewer-only) [W3, DONE, merged; one fix round closed a BLOCKING finding -- tier was excluded from the trigger's freeze-during-publish check, letting a non-authorised reviewer launder a critical claim to publication by downgrading tier in the same update call; new permanent regression test added] -- caught and fixed during the lead's own merge verification (not by any reviewer): the migration's one-time content_hash backfill ran a bare UPDATE against every claims row, which enforce_claims_workflow()'s trigger unconditionally refuses for an already-published row -- invisible to every from-empty clean-rebuild test this migration was verified against, only surfaced against the shared stack's real accumulated data; fixed by disabling both claims triggers around that one backfill statement only. Storage stays off on the shared stack (guarded no-op) pending a separate decision to enable it there.
- [x] PUB-3 - Migration 0018 (real number, not the card's stale "0005" guess): review_events/audit_events (insert-only, reviewer-readable, immutable for every role), saved_plans correction-flag columns, publish_claim() (row-locked, stale-hash guard) and supersede_claim() (same-entity/field only, critical-tier gate, flags affected plans by count only) [W3, DONE, merged] -- **not yet functionally live**: neither function is called anywhere in app/api/claims.py yet, which still does a plain table UPDATE for approve/supersede, so the audit trail and the saved-plans correction-flag protect no real user action until PUB-7 (Claims API completion, already a separately planned card) wires them in. Do not treat the audit trail as live before then.
- [ ] PUB-4 - Owner applies through the current migration ledger after the adversarial review [W3-W4, owner]
- [ ] PUB-13b - Owner: checker account, reviewers rows, critical_authorised on staging [W3-W4, owner, new]
- [x] SCOPE-13 - Reviewer-side currency and jurisdiction integrity [W4, DONE, merged; currency required on money-field claims (422, enforced server-side in the pydantic model), reviewer queue shows jurisdiction/cycle/currency, duplicate-published warning; one fix round closed a live regression both reviewers found -- the AI-extraction flow (the only web UI that can create a claim) had no currency field and started permanently 422ing on every fee/cost proposal] -- disclosed, accepted as-is: currency-required is enforced only at the app layer, not a new DB CHECK; docs/CONTRACTS.md already treats a null-currency money claim as safe-by-design

...
describes (landing -> explore -> compare -> requirements -> timeline).
...
UI-3 adds two more screen modules the same way: `landing_pages.py` (`GET
/`, replacing the old bare redirect to `/explore`) and `start_pages.py`
(the `/start` quick-start question chain).
...
from app.web.start_pages import router as start_pages_router

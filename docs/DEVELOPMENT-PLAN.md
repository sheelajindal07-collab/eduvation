...
| DATA-14 | Owner: apply 0005..N to production with `--through`, run the grant and exposure audit (SEC-6 catalogue test) and a data-security review | owner, 1.5 h, Phase 2 | DEPLOY-11, DATA-11, DATA-13 | CONTENT-10 production import, PUB-13b on production, DEPLOY-12, DEPLOY-13 |
...
1. QA-6 cross-user access matrix is green **in CI** (QA-5) with no-skip mode on, and has rows for every table and policy added by **every migration merged in Phase 1** (the ledger plans 0005-0012).
...
**Float work (never blocks a wave close or this exit gate; unfinished cards roll into P2-A):** AUTH-2, AUTH-3, AUTH-14, CONSENT-6, CONSENT-6b, CONSENT-7, CONSENT-8, CONSENT-10, UI-19. CONSENT-4 (0012) is firm only if the CONSENT-3 human read is done by about day 14; otherwise it floats too. None of these is on the Phase 1 exit path. Each still carries its cross-user acceptance lines (section 5) whenever it is built.
...
- **Owner applies are bounded and tagged.** The lead tags `staging-batch-A` (0005-0006), `-B` (0007-0008), `-C` (0009-0010), `-D` (0011-0012). The owner applies from the tag checkout with `apply_migrations.py --through NNNN` (DATA-15), so a later migration already on `main` can never reach staging early.
...
|---|---|---|---|
| Reviewer calibration (own lane; touches only `tests/calibration/`) | Test engineer, strongest | QA-2, DOCS-4, DOCS-5 done | **QA-9 (ONE session): data-security-reviewer only, 5-6 seeded-flaw SQL and RLS fixtures in `tests/calibration/` (shared later with RULES-13). The 0005-0006 batch review is not requested until this calibration run is recorded.** The ux-qa half is QA-9b in Phase 2. |
| Tests | Test engineer | QA-3 merged | QA-12 (sweep script; the owner runs `--apply` before DATA-10a) ; QA-6 (2 sessions, strongest; **branched only after DATA-12 merges**; written against `main` including 0005 and 0006: 7 tables + `app_settings` + any SCOPE-3 additions; first acceptance line = backfill rows for 0005 and 0006 if they merged first) -> QA-4 (writes `docs/TESTING.md` only; its `CLAUDE.md` and `STATUS.md` changes come back as proposed lines) -> QA-5 (after DEPLOY-3) |
...
| Migration (continuous; DB-touching; own second local stack) | Migration owner: **strongest for SCOPE-3 and AUTH-4** (the two multi-session cards, 2026-09-21 speed change), standard for DATA-15, DATA-12, DATA-8, AUTH-5 | QA-3 merged; contract read-through done for SCOPE-3 | DATA-15 (unit-only, may run during the QA-3 window) -> DATA-12 (0005) -> SCOPE-3 (0006, 2 sessions) -> DATA-8 -> AUTH-4 (0007, 2 sessions; needs your yes on AUTH-1) -> AUTH-5 (0008) -> continues in Wave 3 |
...
- **DATA-10a (owner, end of Wave 2, about day 8-9): run the QA-12 `--apply` sweep, then `apply --through 0006` from the `staging-batch-A` tag, then the synthetic seed.** Only after (a) the data-security batch review of 0005-0006 has no open critical or high finding, (b) revert-to-prove is recorded for the DATA-12 policy and the SCOPE-3 freeze trigger (all earlier columns carried forward), and (c) the QA-6 matrix rows for their tables are green (in CI once QA-5 exists; until then run by the lead on the local stack).
...
| Migration (continuous) | Migration owner, strongest | PUB-2 (0009, 2 sessions, **failing reproduce test first**) -> PUB-3 (0010, 2 sessions) |
...
- **Owner actions due, in order:** **CONTENT-9 maker entry, offline in the CSV (days 11-12, about 2 h a day; moved out of days 8-10)**; **DATA-10b = `apply --through 0008` from the `staging-batch-B` tag, in the same sitting as a `deploy.sh staging` redeploy** (guest save cannot work on staging without 0007 and 0008), only after the batch review of 0007-0008 (AUTH-4 and AUTH-5 RLS, hashed guest token) has no open critical or high finding, revert-to-prove is recorded for the AUTH-4/AUTH-5 RLS, and their matrix rows are green in CI - these are preconditions of the lead's notice, not an afterthought; UI-15 phone walk-through (needs QA-7, DEPLOY-7 and DATA-10b; **day 13-14, which keeps at least one full day of float after the day 8-10 bring-up** - if DEPLOY-7 overruns to day 11-12 it still does not move UI-15 or TRIAL-8); **PUB-4 = `apply --through 0010` from the `staging-batch-C` tag**, within 24 h of the adversarial review of 0009-0010 passing (about day 13-14); then **PUB-13b** (checker account, `reviewers` rows, `critical_authorised`).
...
| Migration (continuous) | Migration owner, strongest | yes | SEC-6 (0011) -> CONSENT-4 (0012, 2 sessions; **only after CONSENT-3 passed its human read**; if that read is late, CONSENT-4 rolls into P2-A and nothing in Phase 1 waits for it) |
...
- Cap: 2-3 writers.
- There is **no migration lane in Wave 5**. AUTH-7 and AUTH-10 are Phase 2 (P2-A); they reach staging with DATA-11. Staging ends Phase 1 on 0005-0012 (or 0005-0011 if CONSENT-4 floated).
...
| 3 | DOCS-3 | Card numbers, migration ledger, stale-name ledger, card stubs. Seven tasks each assumed BCI-007; ten assumed 0005. |
...
| Group | Rule |
|---|---|
| `db/migrations/*`, `apply_migrations.py`, `app/data/models.py`, `tests/db/access_matrix.py` (after QA-6), DATA.md schema section | One migration owner, on its own second local stack. One open migration. Order: DATA-15, then 0005-0012 as listed in section 5. RULES-3 takes its `models.py` slot before SCOPE-3 opens. Additive only. Only the owner applies to cloud projects, with `--through`. |
...
| Adversarial pass on 0009-0010 (PUB-2, PUB-3) before PUB-4. It **is** the batch-C review, not an extra pass | These guard the maker-checker non-negotiable. |
...
| 2 | data-security, batch A | 0005-0006 (DATA-12, SCOPE-3), DATA-8 | Before DATA-10a; after QA-9 calibration |
| 3 | data-security, Wave 2 code | SEC-1, SEC-2, SEC-5, OPS-2, A11Y-4, DEPLOY-15, DEPLOY-9, DEPLOY-6, QA-2/QA-3 guards | Before DEPLOY-7 |
| 4 | data-security, batch B | 0007-0008 (AUTH-4, AUTH-5) and their app code | Before DATA-10b |
| 5 | data-security, Wave 3 code | RULES-8, AUTH-6, CONTENT-6 session 1, CONTENT-4 | Wave 3 close |
| 6 | data-security, batch C, **adversarial** | 0009-0010 (PUB-2, PUB-3) | Before PUB-4 |
| 7 | data-security, batch D | 0011-0012 (SEC-6, CONSENT-4), CONTENT-6b, SCOPE-13 | Before DATA-10c |
...
| 23 | 8-9 | QA-12 sweep `--apply`, then **DATA-10a**: `apply --through 0006` from tag `staging-batch-A`, then the synthetic seed | 1 | 70.2 | Your shell; paste counts and the dry-run "zero matches" line. Only after the lead's notice that review, revert-to-prove and matrix rows are done. | Demo data on staging |
...
| 30 | about 18-20 | **DATA-10c**: `apply --through 0012` from `staging-batch-D` | 0.5 | 86.7 | After its batch review. Skipped if CONSENT-4 floated (then `--through 0011`). | Exit gate item 1 on staging |
...
| 8 | 0007-0008 never reach staging; the apply script cannot stop at a number | DATA-15 (`--through`), tagged batches: DATA-10a (0005-0006), DATA-10b (0007-0008), PUB-4 (0009-0010), DATA-10c (0011-0012) |
| 9 | No task created `migration-owner.md` | DOCS-13 |
| 10 | Console approve path and the importer may break after 0009-0010 | PUB-2 and PUB-3 acceptance lines; CONTENT-6 split so its DB half (CONTENT-6b) is written after PUB-3; slim PUB-7 only if needed |
